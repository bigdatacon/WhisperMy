...
/* Преобразовать распознанный текст в Json для отправки клиенту */
...
/* Сохранить фрагмент звука, подаваемый нв вход нейросети, в .wav */
void save_fragment(const vector<float> & pcmf32) {
...
std::string convert_audio(const vector<float> & pcmf32_new_it, struct whisper_context * ctx, whisper_params & params) {
    static std::vector<float> pcmf32_new;
    static std::vector<float> pcmf32;
    static std::vector<float> pcmf32_old;
...
//    }
    {
        /* Распознаем аудио */
        std::cerr << "Приступил к распознаванию audio..." << std::endl;
        ...
        /* Записать распознаваемый фрагмент в отдельный файл */
        save_fragment(pcmf32);
        ...
        for (int i = 0; i < n_segments; ++i) {
            const char *new_text = whisper_full_get_segment_text(ctx, i);
            text += std::string(new_text) + std::string(" ");
        }
...
int audio_processing_function(int argc, char ** argv, server * serv) {
    ...
    // main audio loop
    while (is_running) {
        std::vector<float> pcmf32_new_it = get_audio();
        wavWriter.write(pcmf32_new_it.data(), pcmf32_new_it.size());
        std::string text = convert_audio(pcmf32_new_it, ctx, params);
        send_text(serv, last_handle, text);
    }


    if (false) {
        std::vector<float> pcmf32, pcmf32_new;
