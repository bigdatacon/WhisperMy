    ...
    //                send_text(serv, last_handle, greet_text);
                ...
                std::cerr << "HERE one Non-zero float  in pcmf32 before cycle: " << non_zero_count_f0 << std::endl;

                for (int i = 0; i < n_samples_take; i++) {
                    pcmf32[i] = pcmf32_old[pcmf32_old.size() - n_samples_take + i];
                }
                size_t non_zero_count_f1 = std::count_if(pcmf32.begin(), pcmf32.end(), [](float val) { return val != 0.0f; });
                std::cerr << "HERE one Non-zero float  in pcmf32 before memcpy : " << non_zero_count_f1 << std::endl;
