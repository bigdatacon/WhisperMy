...
void on_message(int argc, char ** argv,  server* s, websocketpp::connection_hdl hdl, server::message_ptr msg) {
    if (got_config) {
        ...
        json j;
        j["text"] = " p"; // Присваиваем значение переменной ключу "text"
        j["result"] = json::array({
                                          {{"conf", 0.987654321}, {"text", " p"}},
                                          {{"conf", 0.987654321}, {"text", "p "}}
                                  });
        ...
        try {
            ...
            // Проверяем, не является ли указатель пустым и открыто ли соединение
            if (con && con->get_state() == websocketpp::session::state::open) {
                s->send(hdl, greet_text, websocketpp::frame::opcode::text);
            } else {
                std::cerr << "Соединение закрыто или не в состоянии open" << std::endl;
            }
        } catch (const websocketpp::exception& e) {
            std::cerr << "Ошибка при отправке сообщения: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "Неизвестная ошибка при отправке сообщения" << std::endl;
        }

        std::cerr << "приветсвенное сообщение отправлено 1 раз" << std::endl;
...
//    const int n_samples_keep = 480000;
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            std::string greet_text = j.dump();
            try {
                websocketpp::server<websocketpp::config::asio>::connection_ptr con = serv->get_con_from_hdl(last_handle);

                // Проверяем, не является ли указатель пустым и открыто ли соединение
                if (con && con->get_state() == websocketpp::session::state::open) {
                    serv->send(last_handle, greet_text, websocketpp::frame::opcode::text);
                    std::cerr << "приветсвенное сообщение отправлено в цикле" << std::endl;
                } else {
                    std::cerr << "Соединение закрыто или не в состоянии open" << std::endl;
                }
            } catch (const websocketpp::exception& e) {
...
//            std::cerr << "Отправленно приветственное сообщение клиенту в цикле " << std::endl;
            while (true) {
                // приветсвенный текст
